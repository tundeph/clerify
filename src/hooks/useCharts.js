...
export const renderCharts = () => {
		...
		Object.entries(datas)
			...
			.forEach((item) => {
				...
				if (!dailyCategory[item[1].categoryId]) {
					...
					dailyCategory[[item[1].categoryId]][currentKey] +=
						item[1].amount / 100
				} else {
					dailyCategory[[item[1].categoryId]][currentKey] +=
						item[1].amount / 100
	...
	const getMOMData = (
		...
		resultForMOMCategory.datasets[0] = returnData(
			firstData.dailyCategory,
			firstMonthStartDate
		)
		resultForMOMCategory.datasets[1] = returnData(
