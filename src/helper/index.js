  ...
  for (let counter = 0; counter <= Math.abs(keywordsArray.length / 2); counter++) {
    if (keywordsArray[counter] && keywordsArray[counter + 1] && keywordsArray[counter + 2]) {
      threeWordKeywords.push(
        `${keywordsArray[counter]} ${keywordsArray[counter + 1]} ${keywordsArray[counter + 2]}`
      )
    }
  }
...
export const formatCategory = (transactionCategories) => {
  const sortedCategories = textSorter([...transactionCategories], "asc", "title").map(
    (category) => ({
...
export const formatCategoryDropDown = (transactionCategories) => {
  ...
  const categories = [{ value: "", label: "Select a transaction category" }, ...sortedCategories]
  return categories
}
...
export const formatUpdatedCategories = (document, category, keyword, thirdParty) => {
